...
const DoctorAppointments = () => {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      const loadData = async () => {
        try {
          console.log('DoctorAppointments: Refreshing data on focus');
  ...
  const handleRefresh = useCallback(async () => {
    setRefreshing(true);
    try {
      console.log('DoctorAppointments: Manual refresh triggered');
      await refreshAppointments();
    } catch (error) {
  ...
  // Get and sort appointments for the current doctor
  const appointments = useMemo(() => {
    if (!currentUser) {
      console.log('DoctorAppointments: No current user');
      return [];
    }
    
    console.log('DoctorAppointments: Getting appointments for doctor:', currentUser.id);
    console.log('DoctorAppointments: All appointments:', allAppointments);
    
    const doctorAppointments = getAppointmentsByDoctor(currentUser.id);
    console.log('DoctorAppointments: Found doctor appointments:', doctorAppointments);
  ...
  }, [currentUser, allAppointments, getAppointmentsByDoctor]);
 ...
 const { upcomingAppointments, pastAppointments } = useMemo(() => {
  ...
  const upcoming = appointments.filter(apt => {
    try {
      ...
      console.error(`Invalid date/time for appointment ${apt.id}:`, apt);
      return false;
    }
  });
  ...
  // Debug logs
  useEffect(() => {
    console.log('DoctorAppointments: Current user:', currentUser);
    console.log('DoctorAppointments: All appointments in context:', allAppointments);
    console.log('DoctorAppointments: Doctor appointments:', appointments);
    console.log('DoctorAppointments: Upcoming appointments:', upcomingAppointments);
    console.log('DoctorAppointments: Past appointments:', pastAppointments);
  }, [currentUser, allAppointments, appointments, upcomingAppointments, pastAppointments]);

  // Combined loading state
  const isLoading = appointmentsLoading || refreshing;
    ...
    <View style={[globalStyles.container, styles.container]}>
      ...
      <View style={styles.debugContainer}>
        <Text style={styles.debugText}>
          Doctor: {currentUser.name} (ID: {currentUser.id})
        </Text>
        {/* <Text style={styles.debugText}>
